...
```ruby
myString = 'This is "my" String'
myString = 'This is "my" String' # works as well
```
...
- `名前空間とMix-in`: モジュールを重ねることで、MIx-in に使うモジュールを名前空間内で提供することができる。
...
#### Different b/w proc and lambda:

- `Proc` returns from **the current method**, while `lambda` returns from **the lambda itself**
- `Proc` doesn’t care about the correct number of arguments, while `lambda` will raise an exception
- Whenever a `Proc` (including blocks and lambda’s, by extension) proc is instantiated, a `binding` is created which inherits references to the local variables in the context the block was created.
- bindings: 
```ruby
foo = 1
